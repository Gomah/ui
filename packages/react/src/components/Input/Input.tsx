...
const styles = /*tw*/ {
  ...
  variant: {
    ...
    primary: {
      default: 'block rounded-md border-gray-300 focus:border-primary-600 focus:ring-primary-600',
    },
    error: {
...
export const Input = React.forwardRef<HTMLInputElement, React.PropsWithRef<InputProps>>(
  ({ children, className = '', type = 'text', size = 'md', ...props }, ref) => {
    const inputClasses = twMerge(
      clsx(styles.size[size], styles.variant.primary.default, styles.shadow),
      className
    );
